...
class CT5ForConditionalGeneration(T5ForConditionalGeneration):
    ...
    def prepare_inputs_for_generation(
        ...
        return {
            ...
            "decoder_attention_mask": self._get_decoder_attention_mask_from_input_ids(input_ids),
    ...
    def _prepare_decoder_input_ids_for_generation(
            ...
            inputs_tensor: torch.LongTensor = None,
        ...
        if model_kwargs is not None and "decoder_input_ids" in model_kwargs:
            ...
            if device is None:
                device = self.device
            start_id = torch.ones((batch_size, 1), dtype=torch.long, device=self.device) * decoder_start_token_id
    ...
    def _get_interleaved_positional_ids(self, input_ids, eos_token_id: int = 1):
        ...
        for i in range(len(input_ids)):
            ...
            pos = (~sentinels).long().cumsum(-1)
            idxs = sentinels.long().cumsum(-1) - 1
            idxs[idxs < 0] = 0  # the first token is not a sentinel
    ...
    def _get_decoder_causal_mask(self, input_ids, pad_token_id: int = 0):
        ...
        # causal_mask = causal_mask & pads[:, None, :]
        causal_mask[:, :, 0] = True
        return causal_mask

    def _get_decoder_attention_mask_from_input_ids(
            self,
            input_ids: torch.LongTensor,
            eos_token_id: int = 1
    ) -> torch.BoolTensor:
        pad_mask = self._get_decoder_pad_mask(input_ids, eos_token_id)
        causal_mask = self._get_decoder_causal_mask(input_ids)
        return pad_mask[:, None, :] & causal_mask

    def _is_sentinel(self, ids, idx_a=32000, idx_b=32099):
        return (ids >= idx_a) & (ids <= idx_b)

    def _get_decoder_ids_from_input_ids(
            self,
            input_ids: torch.LongTensor,
            pad_token_id: int = 0,
            eos_token_id: int = 1,
        ...
        # get masks for sentinel, pad, and eos tokens
        m_sentinel = self._is_sentinel(input_ids, idx_a, idx_b)
        m_eos = input_ids == eos_token_id
        m_pad = input_ids == pad_token_id

        # m is a long tensor, where
        # 1 denotes non-sentinel tokens,
        # 2 denotes sentinel tokens,
        # 3 denotes eos or pad
        m = (m_sentinel.long() + 1).masked_fill(m_eos | m_pad, 3)

        # get unique sequences and pad with 0
        rev_input_ids = pad_sequence(
            [torch.unique_consecutive(v) for v in m],
            batch_first=True, padding_value=pad_token_id
        )

        # create repeat_interleaved sentinel ids
        batch_size, max_len = rev_input_ids.shape
        ar = torch.arange(idx_b, idx_b - max_len, -1, device=input_ids.device)
        ar_div = torch_int_div(torch.arange(1, max_len + 1, device=input_ids.device), 2)
        sentinel_ids = ar.unsqueeze(0).expand(batch_size, -1) + ar_div.unsqueeze(0)

        # replace non-sentinel tokens by a sentinel token
        decoder_ids = torch.where(rev_input_ids == 1, sentinel_ids, rev_input_ids)
        # replace original sentinel tokens by <pad>
        decoder_ids = decoder_ids.masked_fill(decoder_ids == 2, pad_token_id)
        # recover eos token id
        decoder_ids = decoder_ids.masked_fill(decoder_ids == 3, eos_token_id)
        return decoder_ids
    ...
    def generate(
        ...
        Parameters:
            ...
            parallel_decoding (`bool`, *optional*, defaults to `True`):
                Whether or not to use the fast decoding strategy.
    ...
    def _recover_input_mask_from_input_ids(
            self,
            input_ids: torch.LongTensor,
            pad_mask: torch.BoolTensor,
            pad_token_id: int,
    ):
        # select valid pad positions that are within the sentence
        input_mask = (input_ids == pad_token_id)
        input_mask = input_mask & pad_mask

        # in case the first symbol is a pad token
        input_mask[:, 0] = input_ids[:, 0] != pad_token_id
        return input_mask

    def _recover_next_token_logits_in_parallel(
            self,
    ...
    def _batched_repeat_interleave(self, inp, reps, pad_token_id=0):
        try:
            ...
            batched_repeat_interleave = vmap(torch.repeat_interleave)
    ...
    def _update_input_ids_in_parallel(
            self,
            input_ids: torch.LongTensor,
            input_mask: torch.BoolTensor,
            pad_token_id: int,
        ...
        # compute the number of new positions per element in case the new elements was not eoc
        # if masked position, create a new position, otherwise do nothing
        eoc_mask = input_ids == eoc_token_id
        reps = torch.where(input_mask & (~eoc_mask), 2, 1)

        # repeat_interleave at masked positions
        rep_input_mask = self._batched_repeat_interleave(input_mask, reps, pad_token_id)
        rep_input_ids = self._batched_repeat_interleave(input_ids, reps, pad_token_id)

        # roll mask so that it selects the new inserted positions
        rep_input_mask = rep_input_mask & rep_input_mask.roll(1, dims=-1)

        # replace new inserted positions by <pad>
        rep_input_ids = rep_input_ids.masked_fill(rep_input_mask, pad_token_id)

        return rep_input_ids
    ...
    def greedy_search(
        ...
        cur_len = (input_ids != pad_token_id).sum(-1)
        first_pass = True
        ...
        for _ in range(max_chunk_size):
            ...
            if synced_gpus and this_peer_finished:
                cur_len = cur_len + input_mask.sum(-1)
                continue  # don't waste resources running the code we don't need

            # if is the first pass, should not generate eoc
            if first_pass:
                unfinished_chunks = torch.ones(num_masks, dtype=torch.long, device=input_ids.device)
                outputs.logits[..., eoc_token_id] = -999999.0
                first_pass = False

            # recover next_token_logits and forbid the model to generate ignored_ids
            next_token_logits = self._recover_next_token_logits_in_parallel(
                outputs.logits, input_mask, ignored_ids
            )
            ...
            if eos_token_id is not None:
                ...
                next_tokens = next_tokens * unfinished_chunks + pad_token_id * (1 - unfinished_chunks)

            # replace <pads> by next_tokens
            input_ids[input_mask] = next_tokens[next_tokens != pad_token_id]

            # update generated ids, model inputs, and length for next step
            input_ids = self._update_input_ids_in_parallel(input_ids, input_mask, pad_token_id, eoc_token_id)
            ...
            # if all eoc_tokens were found in one sentence, set sentence to finished
            unfinished_chunks = unfinished_chunks[next_tokens != eoc_token_id]

            # stop when each sentence is finished, or if we exceed the maximum length
            if len(unfinished_chunks) == 0 or stopping_criteria(input_ids, scores):
                if not synced_gpus:
                    break
                else:
    ...
    def beam_search(
        ...
        for _ in range(max_chunk_size):
            ...
            if first_pass:
                ...
                outputs.logits[..., eoc_token_id] = -999999.0
                # create beam scores
                beam_scores = torch.zeros((num_masks // num_beams, num_beams), dtype=torch.float,
                                          device=input_ids.device)
                beam_scores[:, 1:] = -1e9
                beam_scores = beam_scores.view((num_masks,))
                # fix beam scorer
                beam_scorer._beam_hyps = [
                    ...
                    for _ in range(num_masks // num_beams)
                ]
                beam_scorer._done = torch.zeros(num_masks // num_beams, dtype=torch.bool, device=input_ids.device)
                first_pass = False
            ...
            beam_outputs = beam_scorer.process(
                input_ids.repeat_interleave(input_mask.sum(-1), dim=0),
                next_token_scores,
                next_tokens,
                next_indices,
                pad_token_id=pad_token_id,
            ...
            # replace <pads> by next_tokens
            eoc_mask = input_ids == eoc_token_id
            input_ids[input_mask] = beam_next_tokens[beam_next_tokens != pad_token_id]
            input_ids = input_ids.masked_fill(eoc_mask, eoc_token_id)
            ...
            # increase cur_len
            cur_len = cur_len + input_mask.sum(-1)
            unfinished_chunks = unfinished_chunks.mul((beam_next_tokens != eoc_token_id).long())
            # unfinished_chunks = unfinished_chunks[beam_next_tokens != eoc_token_id]

            if unfinished_chunks.max() == 0 or beam_scorer.is_done or stopping_criteria(input_ids, scores):
                if not synced_gpus:
                    break
                else:
                    this_peer_finished = True
        ...
        valid_idxs = [0] + input_mask.sum(-1)[::num_beams].cumsum(-1).tolist()[:-1]
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                return BeamSearchEncoderDecoderOutput(
                    sequences=sequence_outputs["sequences"][valid_idxs],
                    sequences_scores=sequence_outputs["sequence_scores"][valid_idxs],
                    scores=scores[valid_idxs],
                    beam_indices=sequence_outputs["beam_indices"][valid_idxs],
                    encoder_attentions=encoder_attentions[valid_idxs],
                    encoder_hidden_states=encoder_hidden_states[valid_idxs],
