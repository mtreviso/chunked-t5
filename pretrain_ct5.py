...
def compute_input_and_target_lengths_for_chunked_t5(inputs_length, noise_density, mean_noise_span_length):
    """This function is copy of `random_spans_helper <https://github.com/google-research/text-to-text-transfer-transformer/blob/84f8bcc14b5f2c03de51bd3587609ba8f6bbd1cd/t5/data/preprocessors.py#L2466>`__ .
    Training parameters to avoid padding with random_spans_noise_mask.
    When training a model with random_spans_noise_mask, we would like to set the other
    training hyperparmeters in a way that avoids padding.
    ...
    def _tokens_length_to_inputs_length_targets_length(tokens_length):
        ...
        num_noise_spans = int(round(num_noise_tokens / mean_noise_span_length))
        _input_length = num_nonnoise_tokens + num_noise_spans + 1
        # _output_length = tokens_length + num_noise_spans + 1
        _output_length = num_noise_tokens + num_noise_spans * 2 + 1
        return _input_length, _output_length
...
class FlaxDataCollatorForT5MLM:
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> Dict[str, np.ndarray]:
        ...
        last_indices[:, -1] = 0
        last_indices[:, -2] = 1
        labels_sentinel_with_sep = [
            np.insert(labels_ids_sentinel[i], last_indices[i].nonzero()[0] + 1, self.tokenizer.sep_token_id)
        ...
        # batch["labels_weight"] = (label_ids_sentinel == -100) * self.noise_density + (label_ids_sentinel != -100) * 1
        batch["labels_weight"] = (label_ids_sentinel != self.tokenizer.sep_token_id) * 1 + (
                    label_ids_sentinel == self.tokenizer.sep_token_id) * 1
        batch["labels"] = label_ids_sentinel
        ...
        def get_interleaved_positional_ids(decoder_ids):
            ...
            for i in range(len(decoder_ids)):
                count = 0
                pos = []
                sentinels = (decoder_ids[i] >= 32000) & (decoder_ids[i] <= 32099)
                sentinels = sentinels
                for s in sentinels:
                    if s:
                        count = 0
        ...
        encoder_attention_mask = batch["input_ids"] != self.tokenizer.pad_token_id
    ...
    def create_sentinel_ids(self, mask_indices):
        ...
        sentinel_ids = np.where(sentinel_ids != 0, (len(self.tokenizer) - 2 - sentinel_ids), 0)
...
def main():
    ...
    if model_args.config_name:
        ...
        # for some reason vocab_size = 32128 (so we have space for eoc_id and mask_id)
    else:
    ...
    expanded_inputs_length, targets_length = compute_input_and_target_lengths_for_chunked_t5(
    ...
    # Resize embeddings to account for the new special tokens
    # model.resize_token_embeddings(params['shared']['embeddings'].shape[0] + len(new_special_tokens))
    ...
    # custom loss
    def masked_softmax_cross_entropy(logits, labels, labels_weight=None):
        labels_vec = onehot(labels, logits.shape[-1])
        mask = (labels != -100) * 1
        if labels_weight is None:
            masked_sum = jnp.sum(mask[..., None] * labels_vec * jax.nn.log_softmax(logits, axis=-1), axis=-1)
        else:
            masked_sum = jnp.sum(
                mask[..., None] * labels_weight[..., None] * labels_vec * jax.nn.log_softmax(logits, axis=-1), axis=-1)
        return -jnp.sum(masked_sum, axis=-1) / mask.sum(axis=-1)
    ...
    def train_step(state, batch, dropout_rng):
        ...
        def loss_fn(params):
            ...
            labels_weight = batch.pop("labels_weight")

            logits = state.apply_fn(**batch, params=params, dropout_rng=dropout_rng, train=True)[0]

            # compute loss
            # loss = optax.softmax_cross_entropy(logits, onehot(labels, logits.shape[-1])).mean()
            loss = masked_softmax_cross_entropy(logits, labels, labels_weight=labels_weight).mean()
    ...
    def eval_step(params, batch):
        ...
        acc_ex = jnp.sum(jnp.equal(preds, labels) * mask, axis=-1) / jnp.sum(mask, axis=-1)
        accuracy = acc_ex.mean()
